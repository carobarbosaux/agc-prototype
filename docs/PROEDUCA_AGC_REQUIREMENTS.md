...
**Sequential workflow in Canvas:**

1. **AI Generates First** → Content (índice, instrucciones didácticas, temas) is auto-generated
   - AI populates Canvas with initial blocks (for Author role only)
   - Based on didactic instructions for each topic
   - Structure, text, examples are ready
   
2. **Author Edits & Modifies** → Author receives what AI created
   - `editable=true` in Canvas
   - Modifies, corrects, improves blocks
   - Can change block types (p, h1, h2, quote, ul, ol)
   - Full toolbar: AI sections + formatting + comments + notes
   - Action bar: Save draft, Send for review to Coordinator
   
3. **Coordinator Validates** → Approves Author's changes
   - `editable=false` in Canvas
   - Reviews Author's edits
   - Can give feedback via comments
   - Approves or requests corrections
   - Action bar: Send corrections, Approve content
...
2. **Per-topic independent workflows**
   - Each topic has its own cycle: Instructions → Content → Resources (bibiography) → Tests -> mindmaps -> podcast
...
4. **Clear state visibility**
   - States: To start / Locked / Draft / Review / With comments / Approved / Published
...
## Step 3 Canvas Workflow — Detailed Requirements
