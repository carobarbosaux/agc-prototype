...
export const chatHistorial = [
  {
    rol: 'ia',
    mensaje: 'Estoy aquí para ayudarte con el Tema 2: Regresión y clasificación. Puedo generar contenido, mejorar lo que ya tienes, buscar referencias académicas o responder preguntas sobre el tema. ¿Por dónde empezamos?'
  }
]
```
...
- `Topbar` — breadcrumb + estado + notificaciones badge + avatar
- `Sidebar` (Pipeline) — lista de etapas con estados, temas expandibles, navegación
- `ActionBar` — barra fija inferior, acciones variables por rol
- `PanelIA` — panel derecho colapsable con chat

### Componentes de datos
- `EstadoBadge` — badge de color según el estado del sistema de estados
- `GravedadTag` — tag de comentario con color según severidad
